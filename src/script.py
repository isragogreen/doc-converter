...
def commit_and_push(md_path: Path):
    ...
    commit_msg = f"Converted {md_path.name} to MD"
...
def process_file(input_path: Path, output_path: Path):
    ...
    if output_path.exists():
        ...
        if input_time <= output_time:
            print(f"MD для {input_path.name} актуален, пропуск.")
            return
    print(f"Конвертирую {input_path.name}... (RAM: {ram_gb:.1f}GB)")
